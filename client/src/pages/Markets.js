...
export default function Markets() {
  ...
  const fetchMarketPrice = async (currencyPair) => {
    try {
      const response = await axios.get(`https://tradesimserver.onrender.com/api/market-price/${currencyPair}`);
  ...
  useEffect(() => {
    ...
    const fetchMarketPriceAndUpdate = async () => {
      if (selectedSymbol && apiSymbol) {
        try {
          const response = await axios.get(`https://tradesimserver.onrender.com/api/market-price/${apiSymbol}`);
          setMarketData(response.data);
        } catch (error) {
          console.error('Error fetching market price:', error);
        }
  ...
  const handleBuySell = (position) => {
    if (marketData && currentUser && authToken) {
        ...
        fetch('https://tradesimserver.onrender.com/opentrades', requestOptions)
            .then(response => {
                if (!response.ok) {
  ...
  const calculatePNL = async (trade) => {
    try {
      ...
      const response = await fetch(`https://tradesimserver.onrender.com/api/market-price/${currencyPairLowerCase}`);
      if (!response.ok) {
